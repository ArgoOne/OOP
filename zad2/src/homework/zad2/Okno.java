...
public class Okno {//deklaracja publicznej klasy: nazwa klasy jest taka sama jak pliku//
	...
	public Okno(int d, int w, int lSkrzydel){//konstruktor publiczny przyjmujacy argumety typu int//
		...
		skrzydla = new Skrzydlo[lSkrzydel]; //ustawienie skrzydeł nowym obiektem reprezentującym//
		for(int i=0;i<lSkrzydel;i++){ /*pętla dla wartosci poczatkowej =0,
		 bedacej mniejszej niz lSkrzydel, zwiekszajacej sie o 1 */
			skrzydla[i] = new Skrzydlo(d/lSkrzydel,w, new Klamka()); /*tablica dla liczby porządkowej skrzydla=0,1.. 
	...
	public Okno(int d, int w){ //konstruktor publiczny Okno o wartosciach d,w typu int//
		this(d,w,1); //wywołanie innego konstruktora z wartoscia liczby skrzydel=1//
	}	
	public Okno(){ //konstruktor publiczny Okno//
		this(1200,1500,1); //wywołanie innego konstruktora z wartościami d=1200, w=1500, lSkrzydel=1//
	}
	...
	public void setDlugosc(int dlugosc) { //metoda dostępowa ustawiająca wartosc pola dlugosc //
		this.dlugosc = dlugosc; //zmienna dlugosc zostaje przyslonieta polem o tej samej nazwie//
		
		for(int i=0;i<liczbaSkrzydel;i++){ /*pętla dla wartosci poczatkowej =0,
			 bedacej mniejszej niz lSkrzydel, zwiekszajacej sie o 1 */
			skrzydla[i].setDlugosc(dlugosc/liczbaSkrzydel); /*tablica dla liczby porządkowej skrzydla=0,1.. 
			zwrocenie wartosci Dlugosc będącej ilorazem dlugosc i liczbaSkrzydel*/
		}
	}
	...
	public void otworz(){ //metoda publiczna nie zwracjąca niczego//
		for(int i=0;i<liczbaSkrzydel;i++){ /* pętla dla wartosci poczatkowej =0,
			 mniejszej niz liczbaSkrzydel, zwiekszajacej sie o 1 */
	...
	public void zamknij(){ //metoda publiczna nie zwracjąca niczego//
		for(int i=0;i<liczbaSkrzydel;i++){ /* pętla dla wartosci poczatkowej =0,
			 mniejszej niz liczbaSkrzydel, zwiekszajacej sie o 1 */
	...
	public void wypiszStan() { //metoda publiczna nie zwracjąca niczego//
		String tekst = "";
		for(int i=0;i<liczbaSkrzydel;i++){ /* pętla dla wartosci poczatkowej =0,
			 mniejszej niz liczbaSkrzydel, zwiekszajacej sie o 1 */
			tekst += "["+skrzydla[i].stan()+"]"; //co ma być wyświetlane//
		}
		System.out.println(tekst); //Wyświetlenie na konsoli informacji//
	}
}